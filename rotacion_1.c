...
void rotar(double *vector, double angle)
{
  ...
  double vectortemp[4], vectortemp2[4]; 
...
static void display01(void)
{
  ...
  trasladar(VectorGlobal,vectortemp2);//Para calcular la posición del brazo, traslada el vector al centro de la ventana.
  ...
  //glPushMatrix();//Es una pila de matrices para graficar.
    ...
    //El primer segmento del brazo
      ...
      glVertex3f(vectortemp[1], vectortemp[2], vectortemp[3]); // segundo punto de la linea
    glEnd();
    glPopMatrix(); 
    ...
    */
    ...
    glBegin(GL_LINES); 
      ...
      glVertex3f(vectortemp2[0], vectortemp2[1], vectortemp2[2]); //primer punto donde termina el otro
      //esto hay que cambiarlo ^.^
    glEnd();
  glPopMatrix();//Es una pila de matrices para gráficar
   trasladar(VectorGlobal,vectortemp2);
...
static void keyboard01(unsigned char key, int x, int y)
{
   if((key == 0x1b)||(key == 'q')||(key == 'Q'))
     ...
     if ((key=='T') || (key == 't'))//Rotar, se llama función rotar()
       {
         printf("Vector Inicial %f, %f, %f\n",VectorGlobal[0], VectorGlobal[1],VectorGlobal[2]);
         rotar(VectorGlobal, -.1);
         printf("Vector final %f, %f, %f\n",VectorGlobal[0], VectorGlobal[1],VectorGlobal[2]);
